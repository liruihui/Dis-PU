...
from Common.loss_utils import pc_distance,get_repulsion_loss,discriminator_loss,generator_loss
...
class Model(object):
  ...
  def build_model_test(self,final_ratio=4,step_ratio=4):
      ...
      self.G = Generator(self.opts,is_training,name='generator')
  ...
  def train(self):
      ...
      for i in range(restore_epoch, self.opts.training_epoch):
          ...
          self.log_string(
              "epoch %04d g_loss=%.9f  coarse_cd=%.9f  coarse_hd=%.9f  fine_cd=%.9f fine_hd=%.9f  time=%.4f" % (
                  epoch, g_loss, coarse_cd_loss, coarse_hd_loss, fine_cd_loss, fine_hd_loss, duration / 60.0))
  ...
  def train_one_epoch(self,epoch=0):
      ...
      for _ in tqdm(range(n_batches)):
          ...
          self.step += 1
          if  self.step % self.opts.steps_per_print == 0:
