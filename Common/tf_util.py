...
def get_edge_feature_xyz(feat, nn_idx, k):
      """Construct edge feature for each point
      Args:
      ...
      Returns:
        edge features: (batch_size, num_points, k, num_dims)
      """
      og_batch_size = feat.get_shape().as_list()[0]
      ...
      point_cloud_shape = feat.get_shape()
      batch_size = point_cloud_shape[0].value
      num_points = point_cloud_shape[1].value
      num_dims = point_cloud_shape[2].value
      ...
      point_cloud_flat = tf.reshape(feat, [-1, num_dims])
      point_cloud_neighbors = tf.gather(point_cloud_flat, nn_idx+idx_)
      feat_central = tf.expand_dims(feat_central, axis=-2)

      feat_central = tf.tile(feat_central, [1, 1, k, 1])

      edge_feature = tf.concat([feat_central, point_cloud_neighbors-feat_central], axis=-1)
      return edge_feature
